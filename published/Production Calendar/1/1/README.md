## Production Calendar bug-fix version 1.1
...
- PDF: IDTA 02067_Submodel_ProductionCalendar.pdf (unchanged)
- AASX: IDTA 02067_Template_ProductionCalendar.aasx
- JSON: IDTA 02067_Template_ProductionCalendar.json
- AASX: IDTA 02067_Template_ProductionCalendar_forAASMetamodelV3.1.aasx
- JSON: IDTA 02067_Template_ProductionCalendar_forAASMetamodelV3.1.json
...
- Constraint violations in this folder: **8**.

### Known remaining issues

These were not corrected here, because the correct value is not determinable from the template alone.

- **The value must represent a valid file URI scheme according t** (8): File elements reference packaged files as `/aasx/files/...`. This is the convention used across this repository (84 occurrences) and is how AASX Package Explorer addresses files inside the package, but `aas-core3` expects an RFC 8089 `file:` URI. This was left unchanged deliberately: rewriting it would break the packaged-file references that real tooling resolves today. It needs a repository-wide decision, not a per-template fix.
